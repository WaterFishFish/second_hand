...
func DirectPurchase(c *gin.Context) {
	...
	fmt.Println("书籍信息：", buy)

	//获得买家用户（即当前用户）信息
	buyerInfo = logic.GetUserInfoWithLock(c, ts)
	fmt.Println("书籍信息为:", buy.BookName, buy.ISBN, buy.Left)
	fmt.Println("买家信息为：", buyerInfo.Uname, buyerInfo.Balance)
	fmt.Println("卖家姓名为：", buy.SellerName)
	fmt.Println("id为：", buy.ID)
