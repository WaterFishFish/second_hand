...
func GetUserBalance(userName string) (float64, error) {
	...
	err := DB.Where("uname = ?", userName).First(&userInfo).Error
...
func GetUserInfoByNameWithLock(userName string, ts *gorm.DB) *DAO.User {
	...
	//}
	fmt.Println("userName in GetUserInfoByNameWithLock:", userName)
	UserMutex.Lock()
	if err := ts.Debug().Where("uname=?", userName).First(&user).Error; err != nil {
		HandleErrInTransaction(ts, err)
	}
	return user
}
...
func GetSellByIdWithLock(SId int64, ts *gorm.DB) *DAO.Sell {
	...
	//}
	SellMutex.Lock()
	if err := ts.Debug().Where("id = ?", SId).First(sell).Error; err != nil { //查找买家选中的书籍，并将结果存在find中
		HandleErrInTransaction(ts, err)
	}
	return sell
}

func GetSellById(SId int64) *DAO.Sell {
	sell := new(DAO.Sell)
	if err := DB.Debug().Where("id = ?", SId).First(sell).Error; err != nil { //查找买家选中的书籍，并将结果存在sell中
		fmt.Println("error in GetSellById: ", err)
	}
	return sell
}
...
func GetSellByISPWithLock(isbn string, sellerName string, price float64, ts *gorm.DB) (*DAO.Sell, error) {
	...
	//}
	SellMutex.Lock()
	tmp := ts.Debug().Where("isbn = ? AND Seller_Name = ? AND price = ?", isbn, sellerName, price).First(sell)
	if tmp.Error != nil { //查找买家选中的书籍，并将结果存在find中
		HandleErrInTransaction(ts, tmp.Error)
	}
...
// GetSellByISP 根据ISBN，SellerName，price查找唯一对用的出售表项。I：ISBN，S：sellerName，P：price
func GetSellByISP(isbn string, sellerName string, price float64) (*DAO.Sell, error) {
	sell := new(DAO.Sell)
	var err error
	//tmp := ts.Set("gorm:query_option", "FOR UPDATE").
	//	Where("isbn = ? AND Seller_Name = ? AND price = ?", isbn, sellerName, price).First(sell)
	...
	//	HandleErrInTransaction(ts, tmp.Error)
	//}
	tmp := DB.Where("isbn = ? AND Seller_Name = ? AND price = ?", isbn, sellerName, price).First(sell)
...
func GetCartByID(id int) (*DAO.SpCart, error) {
	cart := new(DAO.SpCart)
	var err error
	//tmp := ts.Set("gorm:query_option", "FOR UPDATE").
	//	Where("isbn = ? AND Seller_Name = ? AND price = ?", isbn, sellerName, price).First()
	//if tmp.Error != nil { //查找买家选中的书籍，并将结果存在find中
	//	HandleErrInTransaction(ts, tmp.Error)
	//}
	tmp := DB.Debug().Where("id = ?", id).First(cart)

	if tmp.RowsAffected == 0 { //未查找到对应记录
		err = errors.New("购物车列表中无对应记录")
	}
	return cart, err
}
...
func SaveInfos(info *DAO.ExaggerateInfo) error {
	fmt.Println("***********pageSize in Create:", info.PageSize)

	err := DB.Debug().Where("user_name = ?", info.UserName).Save(info).Error
	return err
}
